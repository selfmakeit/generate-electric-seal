...
public class Seal {
     ...
     * 画公章
     */
    public File getDrawResFile(String pngPath) throws Exception {
    	if (borderSquare != null) {
    		return getDrawPersonalRes(pngPath); // 画私章
    	}
    	...
    	File res = new File(pngPath);
    	
    	boolean b= ImageIO.write(bi, "PNG",res);
    	if(b){
    		return res;
    	}
    	return null;
     ...
     * 画私章
     */
    public File getDrawPersonalRes(String pngPath) throws Exception {
    	if (mainFont == null || mainFont.getText().length() < 2 || mainFont.getText().length() > 4) {
    		throw new IllegalArgumentException("请输入2-4个字");
    	}
